/**
 * Unit tests for the shared email transport's dormant-credential activation.
 *
 * No live email is sent: the Resend call is exercised with an injected `fetch`
 * stub, and the env + dev-log sink are injected too, so we assert the full
 * activation truth-table without touching process globals:
 *   - real key           → attempts a Resend send (dev AND prod)
 *   - real key + non-2xx → throws, and the message leaks neither body nor key
 *   - no key + prod      → throws 'email provider not wired yet', no fetch
 *   - no key + dev       → logs a single greppable line, no fetch
 *   - no key + dev + devLog → the injected line wins, no fetch
 *   - placeholder key    → treated as absent (dev logs / prod throws)
 *
 * Run: node --experimental-strip-types --test apps/web/src/lib/send-email.test.ts
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';

import { isRealCredential, sendEmail, type SendEmailMessage } from './send-email.ts';

const MESSAGE: SendEmailMessage = {
  to: 'wardrobe@example.com',
  subject: 'Hello from Era',
  html: '<p>body</p>',
  text: 'body',
};
const REAL_KEY = 're_live_realkey123';

/** A fetch stub that records its calls and returns a canned Response. */
function stubFetch(status = 200): { fetch: typeof fetch; calls: Array<{ url: string; init?: RequestInit }> } {
  const calls: Array<{ url: string; init?: RequestInit }> = [];
  const fetchImpl = ((input: unknown, init?: RequestInit) => {
    calls.push({ url: String(input), init });
    return Promise.resolve(new Response(status < 300 ? '{"id":"x"}' : 'upstream error', { status }));
  }) as typeof fetch;
  return { fetch: fetchImpl, calls };
}

/** A log sink that records the lines written to it. */
function stubLog(): { log: (m: string) => void; lines: string[] } {
  const lines: string[] = [];
  return { log: (m) => lines.push(m), lines };
}

test('real key: POSTs to Resend with the rendered message (production)', async () => {
  const { fetch, calls } = stubFetch(200);
  await sendEmail(MESSAGE, { env: { RESEND_API_KEY: REAL_KEY, NODE_ENV: 'production' }, fetchImpl: fetch });

  assert.equal(calls.length, 1);
  assert.equal(calls[0]!.url, 'https://api.resend.com/emails');
  const init = calls[0]!.init!;
  assert.equal(init.method, 'POST');
  const headers = init.headers as Record<string, string>;
  assert.equal(headers.Authorization, `Bearer ${REAL_KEY}`);
  const body = JSON.parse(init.body as string);
  assert.equal(body.to, MESSAGE.to);
  assert.equal(body.subject, MESSAGE.subject);
  assert.equal(body.html, MESSAGE.html);
  assert.equal(body.text, MESSAGE.text);
  assert.equal(body.from, 'Era <hello@era.style>'); // default when EMAIL_FROM unset
});

test('real key: honours EMAIL_FROM override, and sends in dev too', async () => {
  const { fetch, calls } = stubFetch(200);
  const log = stubLog();
  await sendEmail(MESSAGE, {
    env: { RESEND_API_KEY: REAL_KEY, EMAIL_FROM: 'Era Team <team@era.style>', NODE_ENV: 'development' },
    fetchImpl: fetch,
    log: log.log,
  });

  assert.equal(calls.length, 1);
  assert.equal(JSON.parse(calls[0]!.init!.body as string).from, 'Era Team <team@era.style>');
  assert.equal(log.lines.length, 0); // sent, not dev-logged
});

test('real key: a non-2xx Resend response throws WITHOUT leaking the body or key', async () => {
  const { fetch } = stubFetch(422);
  await assert.rejects(
    sendEmail(MESSAGE, { env: { RESEND_API_KEY: REAL_KEY, NODE_ENV: 'production' }, fetchImpl: fetch }),
    (error: Error) => {
      assert.ok(!error.message.includes(REAL_KEY), 'error must not contain the api key');
      assert.ok(!error.message.includes(MESSAGE.html), 'error must not contain the email body');
      assert.match(error.message, /status 422/);
      return true;
    },
  );
});

test('no key + production: throws loudly and never calls fetch', async () => {
  const { fetch, calls } = stubFetch(200);
  await assert.rejects(
    sendEmail(MESSAGE, { env: { NODE_ENV: 'production' }, fetchImpl: fetch }),
    /email provider not wired yet/,
  );
  assert.equal(calls.length, 0);
});

test('no key + dev: logs one greppable line and never calls fetch', async () => {
  const { fetch, calls } = stubFetch(200);
  const log = stubLog();
  await sendEmail(MESSAGE, { env: { NODE_ENV: 'development' }, fetchImpl: fetch, log: log.log });

  assert.equal(calls.length, 0);
  assert.equal(log.lines.length, 1);
  assert.match(log.lines[0]!, /^\[era-email\] no provider wired/);
  assert.ok(log.lines[0]!.includes(MESSAGE.subject));
  assert.ok(log.lines[0]!.includes(MESSAGE.to));
});

test('no key + dev + devLog: the injected line wins, no fetch', async () => {
  const { fetch, calls } = stubFetch(200);
  const log = stubLog();
  await sendEmail(MESSAGE, {
    env: { NODE_ENV: 'development' },
    fetchImpl: fetch,
    log: log.log,
    devLog: (_m, write) => write('custom line'),
  });

  assert.equal(calls.length, 0);
  assert.deepEqual(log.lines, ['custom line']);
});

test('placeholder key is treated as absent (dev logs, prod throws)', async () => {
  const { fetch, calls } = stubFetch(200);
  const log = stubLog();
  await sendEmail(MESSAGE, {
    env: { RESEND_API_KEY: 'change-me-resend-api-key', NODE_ENV: 'development' },
    fetchImpl: fetch,
    log: log.log,
  });
  assert.equal(calls.length, 0);
  assert.equal(log.lines.length, 1);

  await assert.rejects(
    sendEmail(MESSAGE, { env: { RESEND_API_KEY: 'change-me-resend-api-key', NODE_ENV: 'production' }, fetchImpl: fetch }),
    /email provider not wired yet/,
  );
});

test('isRealCredential rejects empty and change-me placeholders', () => {
  assert.equal(isRealCredential(undefined), false);
  assert.equal(isRealCredential(''), false);
  assert.equal(isRealCredential('change-me-resend-api-key'), false);
  assert.equal(isRealCredential('re_live_realkey123'), true);
});
